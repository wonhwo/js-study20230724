...
// 회원 목록(ex 4명)에서 회원의 이름과 주소만 추출하고 싶은경우
...
// xxx회원님의 2번째 취미는 xx입니다.를 반복해서 출력
console.log("=========================");

userList
.filter(user=>user.address==='서울')
.map(user=>{return{hobby:user.hobbys[1],name:user.userName};})
.forEach(uhobby=>console.log(`${uhobby.name}회원님의 2번째 취미는 ${uhobby.hobby}`));

// 필터와 맵의 차이
// filter -> 10개짜리 배열에서 조건에 해당하는 것만 필터링
// 필터의 결과 배열은 개수가 10개 이하일 가능성이 크다.

// map -> 10개짜리 배열에서 조건대로 10개 전부 매핑
// 맵의 결과 배열 개수는 배열의 전체 개수다.
