...
let inputName,inputName2,index;
while(true){
    inputName=prompt(`[${idolLsit} ]\n수정할 멤버 이름을 입력해주세요:`);
    if(idolLsit.includes(inputName)){
        index=idolLsit.indexOf(inputName);
        idolLsit.splice(index,1);
        inputName2=prompt(`새로운 이름을 입력해주세요:`);
        idolLsit.splice(index,0,inputName2);
        alert(idolLsit);
        break;
    }
    else{
